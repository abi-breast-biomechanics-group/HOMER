...
```python exec="true" source="above" session="node-indexing"
...
id_mesh = Mesh(nodes=[node0, node1, node2, node3], elements=element)
```
...
elem_left  = MeshElement(node_indexes=[0,1,2,3], basis_functions=H3**2)
elem_right = MeshElement(node_indexes=[1,4,3,5], basis_functions=H3**2)
...
```python exec="true" source="above" session="node-indexing"
# Get a node by ID
node = id_mesh.get_node('corner_00')

# Get an element by ID
elem = id_mesh.get_element('patch')

print(node.id, node.loc, '|', elem.id)
```
...
```python exec="true" source="above" session="node-indexing"
watched = np.array([0, 3])        # two nodes worth keeping track of
