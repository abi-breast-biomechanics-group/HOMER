...
wlocs_def = np.stack((wlocs[:, 0], wlocs[:, 1] + wlocs[:, 0] * 0.1, wlocs[:,2] + 0.1 * wlocs[:, 2]**2), axis=1)
...
grid = mesh0.xi_grid(res=5)
strains = mesh0.evaluate_strain_in_every_element(grid, mesh_def)
slocs = mesh0.evaluate_embeddings(0, grid)
svecs = mesh0.evaluate_jacobians(0, grid)


s = pv.Plotter()
mesh0.plot(s, mesh_opacity=0.1)
